This app is based on udemy course
https://www.udemy.com/course/android-beginners-weather-forecast-app/
Project was amended with a Retrofit, Hilt and MVVM.
Unresolved exception remains - Hilt cannot provide ViewModelProviderFactory.

API for the project - https://openweathermap.org/current
Test request on a following link - http://api.openweathermap.org/data/2.5/weather?q=Kazan&appid=a8f0e797059b7959399040200fd43231

JSON data to data class - https://app.quicktype.io/

- Resolve an exception on a Hilt's viewModelFactory providing
- Make ResponseToResourceConverter to have a different model for domain layer
- Add fragment for presentation