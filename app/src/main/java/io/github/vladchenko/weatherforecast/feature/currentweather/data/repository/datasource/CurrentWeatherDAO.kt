 ...
 *
 * Provides methods to insert, update, delete, and query [io.github.vladchenko.weatherforecast.feature.currentweather.data.model.CurrentWeatherEntity] objects
 * using Room persistence library. Supports both one-shot operations and reactive data streams.
 *
 * All suspend functions must be called from a coroutine context. Queries returning [kotlinx.coroutines.flow.Flow]
 * emit new data whenever the underlying database changes, making them suitable for UI observation.
 *
 * The table name `citiesForecasts` is defined in the [io.github.vladchenko.weatherforecast.feature.currentweather.data.model.CurrentWeatherEntity] class.
 */
@Dao
interface CurrentWeatherDAO {
    ...
    /**
     * Retrieves all stored weather forecasts as a cold [kotlinx.coroutines.flow.Flow].
     *
